...
const createPharmacy = async (req, res) => {
  ...
  try {
    const [facilityEmailCheck, adminEmailCheck, licenseCheck] = await Promise.all([
      client.query(`SELECT id FROM pharmacies WHERE LOWER(TRIM(email))=LOWER(TRIM($1)) AND deleted_at IS NULL LIMIT 1`, [cleanEmail]),
      client.query(`SELECT id FROM users WHERE LOWER(TRIM(email))=LOWER(TRIM($1)) LIMIT 1`, [cleanAdminEmail]),
      license_number
        ? client.query(`SELECT id FROM pharmacies WHERE LOWER(TRIM(license_number))=LOWER(TRIM($1)) AND deleted_at IS NULL LIMIT 1`, [license_number.trim()])
    ...
    `, [name.trim(), cleanEmail, phone ? phone.trim() : null, address ? address.trim() : null, city ? city.trim() : null, country || 'Kenya', license_number ? license_number.trim() : null, type]);
    ...
    const adminPerms = ROLE_PERMISSIONS['facility_admin'] || [];
    const salt = await bcrypt.genSalt(12);
    const hashedPassword = await bcrypt.hash(admin_password, salt);
    const adminResult = await client.query(`
      INSERT INTO users (full_name, email, password, role, pharmacy_id, permissions)
      VALUES ($1,$2,$3,'facility_admin',$4,$5) RETURNING id, full_name, email, role
    `, [admin_name?.trim() || `${name} Admin`, cleanAdminEmail, hashedPassword, pharmacy.id, JSON.stringify(adminPerms)]);
    ...
    const departments = [
      ...
      { name: 'Inpatient', type: 'inpatient' },
      { name: 'Theatre', type: 'theatre' },
      { name: 'Radiology', type: 'radiology' }
    ];
    for (const dept of departments) {
      await client.query(`
        INSERT INTO departments (name, type, pharmacy_id)
        VALUES ($1, $2, $3)
        ON CONFLICT DO NOTHING
      `, [dept.name, dept.type, pharmacy.id]);
    }
    ...
    logger.info(`Facility created and committed: ${name} (${type})`);
    ...
    if (!targetUserId) {
      ...
      if (adminRes.rows.length > 0) {
        ...
        if (anyUser.rows.length > 0) {
          targetUserId = anyUser.rows[0].id;
        } else {
          // Fallback check: find pharmacy email and match in users
          const pRes = await pool.query(`SELECT email FROM pharmacies WHERE id = $1`, [pharmacy_id]);
          if (pRes.rows.length > 0 && pRes.rows[0].email) {
            const uByEmail = await pool.query(`SELECT id FROM users WHERE LOWER(email) = LOWER($1) LIMIT 1`, [pRes.rows[0].email]);
            if (uByEmail.rows.length > 0) targetUserId = uByEmail.rows[0].id;
          }
        }
      }
    }
