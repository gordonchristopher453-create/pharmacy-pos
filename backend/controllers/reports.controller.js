...
const getLabSalesSummary = async (req, res) => {
  try {
    ...
    const pid  = req.pharmacy_id;
    const result = await pool.query(`
      SELECT lr.test_name, COUNT(*) AS count, SUM(bi.total_price) AS total
      FROM billing_items bi
      JOIN lab_requests lr ON bi.service_order_id::text = lr.service_order_id::text
      WHERE (bi.facility_id::text=$1::text OR bi.facility_id IS NULL) AND bi.item_type='lab' AND DATE(bi.created_at) BETWEEN $2 AND $3
      GROUP BY lr.test_name ORDER BY total DESC
    `, [pid, from, to]);
