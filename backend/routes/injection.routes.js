    ...
    let query = `SELECT v.*, p.full_name as patient_name, p.patient_number, p.gender, p.date_of_birth, p.phone,
      ...
      WHERE v.pharmacy_id = $1 ${dateWhere}
        AND EXISTS (
          ...
          WHERE iro2.visit_id = v.id
        )
        AND (
          LOWER(v.status) NOT IN ('injection_room', 'waiting_injection')
          OR NOT EXISTS (
            SELECT 1 FROM injection_room_orders iro3
            WHERE iro3.visit_id = v.id AND LOWER(iro3.status) = 'pending'
          )
        )`;
    if (search) {
      params.push(`%${search}%`);
...
router.get(['/visit/:visit_id', '/visit/:visit_id/orders'], async (req, res) => {
  try {
    const result = await pool.query(`
      ...
      LEFT JOIN users u ON iro.administered_by = u.id
      LEFT JOIN users p ON iro.prescribed_by = p.id
      WHERE iro.visit_id = $1 AND iro.pharmacy_id = $2
      ORDER BY iro.created_at DESC
...
router.post('/visit/:visit_id/orders', async (req, res) => {
  try {
    ...
    const dup = await pool.query(`
      ...
      WHERE visit_id=$1 AND pharmacy_id=$2 AND LOWER(drug_name)=LOWER($3)
  ...
  try {
    ...
    try {
      ...
      const pendingCheck = await client.query(
        "SELECT COUNT(*)::int AS cnt FROM injection_room_orders WHERE visit_id=$1 AND LOWER(status)='pending'",
        [o.visit_id]
      );
