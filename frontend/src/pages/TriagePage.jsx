  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-[var(--bg-base)] text-[var(--text-primary)] font-sans">
      ...
      <main className="flex-1 overflow-hidden flex flex-col md:flex-row">
        ...
        {/* LEFT COMPONENT: QUEUE NAVIGATION DRAWER / LIST */}
        <div className={`w-full md:w-[380px] border-r border-[var(--border)] bg-[var(--bg-surface)] flex flex-col flex-shrink-0 ${
          selected ? 'hidden md:flex' : 'flex'
        }`}>
          {/* Quick Filter */}
          <div className="p-4 border-b border-[var(--border)] space-y-3">
            <span className="text-xs font-mono font-bold uppercase tracking-widest text-[var(--accent)] block">
              {tab === 'queue' ? '📋 Pending Worklist' : '🔍 Lookup Past Encounters'}
            </span>
            <div className="relative">
              ...
              <input 
                ...
                placeholder="Patient name, record #, phone..."
            ...
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <input 
                  ...
                  className="bg-[var(--bg-elevated)] border border-[var(--border)] rounded-lg px-2 py-1 text-xs text-[var(--text-primary)] focus:outline-none"
                />
              </div>
              {tab === 'queue' && (
                <div>
            ...
            {tab === 'queue' ? (
              loading ? (
                <div className="flex flex-col items-center justify-center py-20 text-[var(--text-muted)]">
                  <Loader className="animate-spin text-[var(--accent)] mb-3" size={24} />
                  <p className="text-xs">Fetching active triage list...</p>
                </div>
              ) : filtered.length === 0 ? (
                <div className="text-center py-16 px-4 border border-dashed border-[var(--border)] rounded-2xl">
                  <div className="text-3xl mb-2">🎉</div>
                  <h4 className="font-bold text-sm text-[var(--text-primary)]">Triage Queue Cleared</h4>
                  <p className="text-xs text-[var(--text-muted)] mt-1">There are no pending patients waiting for triage at this hour.</p>
                </div>
              ) : (
                filtered.map((v, index) => {
                  const pr = PRIORITY.find(p => p.value === v.priority) || PRIORITY[0];
                  const isCur = selected && selected.id === v.id;
                  ...
                  return (
                    <div 
                      ...
                      className={`group border rounded-xl overflow-hidden cursor-pointer transition-all ${
                        isCur 
                          ? 'border-[var(--accent)] bg-[var(--bg-elevated)] ring-1 ring-[var(--accent)]/30' 
                          : 'border-[var(--border)] hover:border-[var(--text-muted)] bg-[var(--bg-surface)] hover:bg-[var(--bg-elevated)]/20'
                      }`}
                    ...
                    >
                      <div className="p-3.5 space-y-2">
                        <div className="flex justify-between items-start gap-1">
                          <div className="space-y-0.5">
                            ...
                            <div className="flex items-center gap-2 text-[10px] font-mono text-[var(--text-muted)]">
                              <span className="text-[var(--accent)]">{v.patient_number}</span>
                              <span>•</span>
                              <span>{getAge(v.date_of_birth)}</span>
                            </div>
                          </div>
                          <span className="text-[10px] font-mono bg-[var(--bg-elevated)] border border-[var(--border)] text-[var(--text-muted)] px-1.5 py-0.5 rounded-md">
                            #{index + 1}
                          </span>
                        </div>
                        ...
                        <div className="flex justify-between items-center pt-1.5 border-t border-[var(--border)]/40 text-[10px]">
                          <span className="font-bold tracking-wider uppercase flex items-center gap-1" style={{ color: pr.color }}>
                            <span>{pr.icon}</span> {pr.value}
                          </span>
                          <span className={`font-mono flex items-center gap-1 ${longWait ? 'text-rose-400 font-bold animate-pulse' : 'text-[var(--text-muted)]'}`}>
                            <Clock size={11} />
                            {waitMins < 60 ? `${waitMins}m` : `${Math.floor(waitMins / 60)}h ${waitMins % 60}m`} wait
                          </span>
                        </div>
                      </div>
                    </div>
              ...
              histLoading ? (
                ...
                <div className="text-center py-16 px-4 border border-dashed border-[var(--border)] text-[var(--text-muted)] rounded-2xl">
                  <p className="text-xs">No records found for {new Date(histDate).toLocaleDateString()}</p>
                </div>
              ) : (
                filteredHistory.map(v => {
                  ...
                  return (
                    <div 
                      key={v.id}
                      className="p-3 bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl space-y-2 text-xs"
                      style={{ borderLeftWidth: '3.5px', borderLeftColor: pr.color }}
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          <h5 className="font-bold text-[var(--text-primary)]">{v.patient_name}</h5>
                          <p className="text-[10px] font-mono text-[var(--text-muted)]">{v.patient_number} · {v.gender}</p>
                        </div>
                        <span className={`px-2 py-0.5 rounded-full font-bold text-[9px] border ${stColorClass}`}>
                          {STATUS_LABELS[v.status] || v.status.toUpperCase()}
                        </span>
                      </div>
                      
                      {v.chief_complaint && (
                        <p className="text-[11px] text-[var(--text-muted)] bg-[var(--bg-elevated)]/40 p-1.5 rounded border border-[var(--border)]/30">
                          {v.chief_complaint}
                        </p>
                      )}

                      <div className="flex justify-between items-center pt-1 text-[10px] text-[var(--text-faint)] font-mono">
                        <span>{dept ? `${dept.icon} ${dept.label}` : '—'}</span>
                        <span>{new Date(v.visit_date || v.created_at).toLocaleTimeString('en-KE', { hour: '2-digit', minute: '2-digit' })}</span>
                      </div>
                    </div>
                  );
                })
        ...
        {/* RIGHT COMPONENT: ACTIVE CLINICAL WORKSHEET / DETAILS */}
        <div className={`flex-1 overflow-y-auto bg-[var(--bg-base)] flex flex-col ${
          selected ? 'flex' : 'hidden md:flex items-center justify-center'
        }`}>
          {!selected ? (
            <div className="p-8 text-center max-w-md space-y-4">
              <div className="w-16 h-16 rounded-2xl bg-[var(--bg-elevated)] border border-[var(--border)] flex items-center justify-center text-3xl mx-auto shadow-inner">
                🩺
              </div>
              <div>
                <h3 className="font-black text-lg text-[var(--text-primary)]">Triage Workspace Ready</h3>
                <p className="text-xs text-[var(--text-muted)] mt-1.5 leading-relaxed">
                  Select a patient record from the waitlist on the left to initialize the clinical examination, record physiological vitals, and issue routing directions.
                </p>
              </div>
              <div className="flex items-center justify-center gap-2.5 text-[11px] text-[var(--accent)] font-mono font-bold bg-[var(--accent)]/5 border border-[var(--accent)]/15 px-3 py-1.5 rounded-xl w-fit mx-auto">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-ping" />
                Standby: Nurse Station Ready
              </div>
            </div>
          ) : (
            <div className="p-6 space-y-6">
              
              {/* BACK ACTION & WORKSTATION BANNER */}
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-[var(--bg-surface)] p-4 rounded-2xl border border-[var(--border)]">
                <div className="flex items-center gap-3">
                  <button 
                    onClick={() => setSelected(null)}
                    className="md:hidden p-2 bg-[var(--bg-elevated)] border border-[var(--border)] rounded-xl text-[var(--text-primary)]"
                  >
                    <ChevronLeft size={16} />
                  </button>
                  <div className="space-y-0.5">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-8 space-y-6">
                  ...
                  <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-2xl overflow-hidden shadow-sm">
                    ...
                    {/* ACTIONS FOOTER */}
                    <div className="p-4 border-t border-[var(--border)] bg-[var(--bg-elevated)]/40 flex justify-end gap-3">
                      <button 
                        type="button"
                        onClick={() => setSelected(null)}
                        className="px-5 py-2.5 text-xs font-bold rounded-xl bg-transparent hover:bg-[var(--bg-surface)] text-[var(--text-primary)] border border-[var(--border)] transition-all"
                      >
                        Cancel
                      </button>
                      <button 
