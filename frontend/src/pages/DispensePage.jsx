...
export default function DispensePage({ user: propUser }) {
  ...
  const fetchRxQueue = useCallback(async (silent = false) => {
    ...
    try {
      ...
      params.append('include_inpatient', 'false');
      ...
      try {
        ...
        res = await api.get('/pharmacy/queue?include_inpatient=false');
      }
  ...
  const openRx = (rx) => {
    setSelectedRx(rx);
    const isInpatient = tab === 'inpatient' || Boolean(rx.is_inpatient && (rx.ward_name || rx.bed_number));
