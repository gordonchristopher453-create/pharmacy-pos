...
export default function PatientsPage() {
  ...
  const fetchAll = () => { fetchPatients(); fetchQueue(); };
    ...
    <div className="h-screen flex flex-col overflow-hidden bg-[var(--bg-surface)]/20">
      ...
      <div className="p-6 border-b border-[var(--border)] bg-[var(--bg-surface)] flex-shrink-0 shadow-sm relative z-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mt-6">
          {[
            { label:'Total Registered Database', value: stats.total_patients||0, color:'var(--accent)', icon: Users, desc: 'Database profiles' },
        ...
        <div className="flex gap-1 mt-6 bg-[var(--bg-elevated)] rounded-xl p-1 w-full sm:w-fit border border-[var(--border)]">
          {[
            { id:'queue', label:'💰 active billing Queue', count: visits.length, icon: CreditCard },
            { id:'patients', label:'👥 Master patient database', count: stats.total_patients, icon: Users },
            { id:'history', label:'📋 System history Logs', count: null, icon: Clock },
          ].map(t => {
            ...
            return (
              <button 
                key={t.id} 
                onClick={() => setTab(t.id)} 
                className={`flex-1 sm:flex-none py-2 px-4 rounded-lg font-bold text-xs uppercase tracking-wider border-none cursor-pointer transition-all duration-200 flex items-center justify-center gap-2 ${
                  isActive 
              ...
              >
                ...
                {t.count !== null && t.count !== undefined && (
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-md font-mono ${isActive ? 'bg-[#0F1612]/20 text-[#0F1612]' : 'bg-[var(--bg-surface)] text-[var(--text-muted)]'}`}>
                    {t.count}
                  </span>
                )}
              </button>
